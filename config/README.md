...
Build your own image with the [`Dockerfile`](../containers/apt/Dockerfile) in `containers/apt/` and set `containers.apt` to either a registry reference or the path of a local `.sif` file.
