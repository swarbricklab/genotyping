...
rule generate_cel_file_list_all:
    ...
    output:
        cel_list=genotyping/"cel_list.txt"
    log:
...
rule apt:
    ...
    output:
        axiom_log=genotyping/"apt/apt-genotype-axiom.log",
        axiom_calls=genotyping/"apt/AxiomGT1.calls.txt",
        axiom_confidences=genotyping/"apt/AxiomGT1.confidences.txt",
        axiom_report=genotyping/"apt/AxiomGT1.report.txt",
        axiom_snp_posteriors=genotyping/"apt/AxiomGT1.snp-posteriors.txt",
        axiom_summary=genotyping/"apt/AxiomGT1.summary.a5",
        axion_chp_bin=genotyping/"apt/AxiomAnalysisSuiteData/All_genotypes_by_snps.CHP.bin",
        axion_chp_index=genotyping/"apt/AxiomAnalysisSuiteData/All_genotypes_by_snps.CHP.index.txt"
    params:
        dir_annotation="resources/genotyping/annotation/",
        dir_output=genotyping/"apt/"
    log:
...
rule ps_metrics:
    input:
        ...
        axiom_calls=genotyping/"apt/AxiomGT1.calls.txt"
    output:
        metrics=genotyping/"ps_metrics/metrics.txt"
    log:
...
rule ps_classification:
    ...
    output:
        ...
        recommended=genotyping/"ps_classification/Recommended.ps"
    params:
        dir_output=genotyping/"ps_classification/"
    log:
...
rule otv_caller:
    ...
    params:
        dir_batch=genotyping/"apt/",
        dir_output=genotyping/"otv_caller/"
    log:
...
rule apt_format_result:
    input:
        ...
        otv_keep=genotyping/"otv_caller/OTV.keep.ps"
    output:
        vcf_file=genotyping/"apt_format_result/combined.b37.vcf"
    params:
        dir_batch=genotyping/"otv_caller/",
